...
${EXECUTED_STEPS}
======================作成・変更したファイル======================
- /etc/httpd/conf/httpd.conf (AllowOverride All、ServerTokens、ServerSignatureを変更)
...
- /home/unicorn/setup_angular.sh (Angularプロジェクト初期化スクリプト)
- /var/www/${PROJECT_NAME}/ (Angularプロジェクトディレクトリ、所有者unicorn:apache)
- /var/www/html (ドキュメントルート、Angularビルド成果物に置換、所有者unicorn:apache)
- /var/www/html/.htaccess (Angular SPA用ルーティング設定)
- firewalld 永続ルール: http、https サービスを許可
...
SUMMARYEOF
        ...
        warn_message "このスクリプトはRHEL/CentOS 8、9または10系のみ対応しています。"
        warn_message "検出されたバージョン: ${DIST_NAME} ${DIST_VERSION_ID}"
        exit 1
    fi
else
    warn_message "このスクリプトはRHEL系のみ対応しています。"
    warn_message "対応ディストリビューション: AlmaLinux、Rocky Linux、RHEL、CentOS Stream、Oracle Linux (8/9/10系)"
    exit 1
fi
